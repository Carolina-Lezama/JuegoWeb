import { objetosDelPersonaje, setPuntosTotales,puntos,puntosTotales, actualizarPuntosTotales, jugador,datosObjetos,objetos,objetosActivos, personajeHumanoEnUso, personajeGatoEnUso, ApartadoMenu, setPersonajeHumanoEnUso, setPersonajeGatoEnUso, setApartadoMenu,objetos_jugador } from '../globals.js';
...
export class EscenaCementerio  extends Phaser.Scene {
    constructor() {
        super({ key: 'EscenaCementerio' });
  ...
  mostrarImagenesVictoria(){
    ...
    PausaPartida() {
                console.log('Regreso al menú principal');
  ...
  create() {
    ...
    this.Calavera = this.add.sprite(0, 0, 'Calavera');

    // Crear animaciones solo si no existen
    if (!this.anims.exists('calavera')) {
        this.anims.create({
            key: 'calavera',
    ...
    this.Calavera.setPosition(this.scale.width * 0.8, this.scale.height * 0.75);
    this.Calavera.setVisible(true);
    this.Calavera.setDepth(5);
    this.Calavera.setScale(1.2);
    this.Calavera.anims.play('calavera', true);



    // Crear enemigos con posiciones visibles y configuración adecuada
    this.enemigos = [
        { sprite: this.Calavera, vida: 80, id: 'Calavera', retroceso: 0 },
    ];

    // Configurar física de enemigos
    this.physics.add.existing(this.Calavera);
    this.Calavera.body.setImmovable(false);
    ...
    const map = this.make.tilemap({ key: 'CementerioJSON' });
    const tileset = map.addTilesetImage('Cementerio', 'Cementerio');
    ...
    this.enemigos.forEach((enemigo) => {
        ...
        if (enemigoEnRango) {
            ...
            if (enemigo.vida <= 0) {
                ...
                setPuntosTotales(puntosTotales + 35);
  ...
  update(time, delta) {
    ...
    this.vidaPersonaje = 100;
    window.ultimaEscenaActiva='EscenaCementerio';
    }
  }
}
