...
export class EscenaPeleaSlime  extends Phaser.Scene {
    constructor() {
        super({ key: 'EscenaPeleaSlime' });
  ...
  create() {
    ...
    this.SlimeVerde.setPosition(this.scale.width * 0.8, this.scale.height * 0.75);
    this.SlimeVerde.setVisible(true);
    this.SlimeVerde.setDepth(5);
    this.SlimeVerde.setScale(0.75);
    this.SlimeVerde.anims.play('slimeVM', true);

    this.SlimeRojo.setPosition(this.scale.width * 0.7, this.scale.height * 0.75);
    this.SlimeRojo.setVisible(true);
    this.SlimeRojo.setDepth(5);
    this.SlimeRojo.setScale(0.8);
    this.SlimeRojo.anims.play('slimeRM', true);

    // Crear enemigos con posiciones visibles y configuración adecuada
    this.enemigos = [
        { sprite: this.SlimeVerde, vida: 25, id: 'slimeVerde', retroceso: 0 },
        { sprite: this.SlimeRojo, vida: 25, id: 'slimeRojo', retroceso: 0 }
    ];

    // Configurar física de enemigos
    this.physics.add.existing(this.SlimeVerde);
    this.physics.add.existing(this.SlimeRojo);
    this.SlimeVerde.body.setImmovable(false);
    this.SlimeRojo.body.setImmovable(false);
    ...
    const map = this.make.tilemap({ key: 'EscenaPeleaSlimeJson' });
    const tileset = map.addTilesetImage('EscenaPeleaSlime', 'EscenaPeleaSlime');
    ...
    this.enemigos.forEach((enemigo) => {
        ...
        if (enemigoEnRango) {
            ...
            if (enemigo.vida <= 0) {
                ...
                setPuntosTotales(puntosTotales + 15);
  ...
  update(time, delta) {
    ...
    this.vidaPersonaje = 100;
    window.ultimaEscenaActiva='EscenaPeleaSlime';
    }
  }
}
