...
export class EscenaCasaAbandonada  extends Phaser.Scene {
    constructor() {
        super({ key: 'EscenaCasaAbandonada' });
  ...
  create() {
    ...
    this.Duende = this.add.sprite(0, 0, 'Duende');

    // Crear animaciones solo si no existen
    if (!this.anims.exists('duende')) {
        this.anims.create({
            key: 'duende',
    ...
    this.Duende.setPosition(this.scale.width * 0.8, this.scale.height * 0.75);
    this.Duende.setVisible(true);
    this.Duende.setDepth(5);
    this.Duende.setScale(0.6);
    this.Duende.anims.play('duende', true);



    // Crear enemigos con posiciones visibles y configuración adecuada
    this.enemigos = [
        { sprite: this.Duende, vida: 35, id: 'Duende', retroceso: 0 },
    ];

    // Configurar física de enemigos
    this.physics.add.existing(this.Duende);
    this.Duende.body.setImmovable(false);
    ...
    const map = this.make.tilemap({ key: 'CasaAbandonadaJSON' });
    const tileset = map.addTilesetImage('CasaAbandonada', 'CasaAbandonada');
    ...
    this.enemigos.forEach((enemigo) => {
        ...
        if (enemigoEnRango) {
            ...
            if (enemigo.vida <= 0) {
                ...
                setPuntosTotales(puntosTotales + 20);
